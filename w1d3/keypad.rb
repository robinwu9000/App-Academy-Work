...
class KeypadTester
  ...
  def run_optimizer
    ...
    until @keypad_instance.all_codes_entered? || codes_left.empty?
      current_code = codes_left.shift
      current_code.each do |key|
        @keypad_instance.press(key)
      end
      mode = @mode_keys[rand(@mode_keys.length)]
      @keypad_instance.press(mode)
      #p "current code just entered: #{current_code}"
      
      current_code = current_code[-(@code_length - 1)..-1] << mode
      #p "random numbered: #{current_code}"
      while codes_left.include?(current_code) do
        mode = @mode_keys[rand(@mode_keys.length)]
        @keypad_instance.press(mode)
        codes_left.delete(current_code)
        #p "before change: #{current_code}"
        current_code = current_code[-(@code_length - 1)..-1] << mode
        #p current_code
      end
    end
